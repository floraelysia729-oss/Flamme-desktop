...
export async function bootDesktop(
  ...
  if (!backendOk) {
    const { getSidecarStatus, formatSidecarBootError } = await import('./bridge')
    const st = await getSidecarStatus()
    const err = formatSidecarBootError(st)
    onPhase?.('error', err)
    return { ok: false, error: err }
  }
  ...
  if (vaultPath && isVaultMode()) {
    try {
      const { useVaultUiStore } = await import('../vault/vaultUiStore')
